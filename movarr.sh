...
validateConfiguration() {
    ...
    # Validate minFreeDiskSpace (should be a number followed by MB or GB, with optional space)
    logMessage "debug" "minFreeDiskSpace: $minFreeDiskSpace"
    if [[ "$minFreeDiskSpace" =~ ^([0-9]+)[[:space:]]*(MB|GB)?$ ]]; then
        size=${BASH_REMATCH[1]}
        unit=${BASH_REMATCH[2]}
        if [ -z "$unit" ]; then
            unit="GB"
            logMessage "debug" "No unit provided for minFreeDiskSpace, defaulting to GB"
        fi
        if [ "$unit" == "GB" ]; then
            minFreeDiskSpace=$((size * 1024))
            logMessage "debug" "Converted minFreeDiskSpace to MB: $minFreeDiskSpace"
        else
            minFreeDiskSpace=$size
        fi
    else
        logMessage "error" "Invalid value for 'minFreeDiskSpace'. It should be a number followed by MB or GB."
        ((errors++))
    fi

    # Validate maxSourceDiskFreeSpace (should be a number followed by MB or GB, with optional space)
    logMessage "debug" "maxSourceDiskFreeSpace: $maxSourceDiskFreeSpace"
    if [[ "$maxSourceDiskFreeSpace" =~ ^([0-9]+)[[:space:]]*(MB|GB)$ ]]; then
        size=${BASH_REMATCH[1]}
        unit=${BASH_REMATCH[2]}
        if [ -z "$unit" ]; then
            unit="GB"
            logMessage "debug" "No unit provided for maxSourceFreeDiskSpace, defaulting to GB"
        fi
        if [ "$unit" == "GB" ]; then
            maxSourceDiskFreeSpace=$((size * 1024))
            logMessage "debug" "Converted maxSourceDiskFreeSpace to MB: $maxSourceDiskFreeSpace"
        else
            maxSourceDiskFreeSpace=$size
        fi
    else
        logMessage "error" "Invalid value for 'maxSourceDiskFreeSpace'. It should be a number followed by MB or GB."
        ((errors++))
    fi

    # Validate minTargetDiskFreeSpace (should be a number followed by MB or GB, with optional space)
    logMessage "debug" "minTargetDiskFreeSpace: $minTargetDiskFreeSpace"
    if [[ "$minTargetDiskFreeSpace" =~ ^([0-9]+)[[:space:]]*(MB|GB)$ ]]; then
        size=${BASH_REMATCH[1]}
        unit=${BASH_REMATCH[2]}
        if [ -z "$unit" ]; then
            unit="GB"
            logMessage "debug" "No unit provided for minTargetFreeDiskSpace, defaulting to GB"
        fi
        if [ "$unit" == "GB" ]; then
            minTargetDiskFreeSpace=$((size * 1024))
            logMessage "debug" "Converted minTargetDiskFreeSpace to MB: $minTargetDiskFreeSpace"
        else
            minTargetDiskFreeSpace=$size
        fi
    else
        logMessage "error" "Invalid value for 'minTargetDiskFreeSpace'. It should be a number followed by MB or GB."
        ((errors++))
    fi
...
transferDirectories() {
    # local sourceDirs=("$@")
    # local destinationDisks=("/mnt/disk1" "/mnt/disk2" "/mnt/disk3") # Example destination disks
    local requiredSpace
